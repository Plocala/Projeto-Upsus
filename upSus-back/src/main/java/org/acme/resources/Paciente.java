...
import br.unitins.topicos1.service.PacienteServiceImpl;

@Path("/autorManga")
public class PacienteResource {
    
    @Inject PacienteServiceImpl autorMangaService;
    ...
    @POST
    public Response create(PacienteDTO autorManga) {
        return Response.status(Status.CREATED).entity(autorMangaService.create(autorManga)).build();
    }

    @PUT
    @Path("/{id}")
    public Response update(@PathParam("id") long id, PacienteDTO autorManga) {
